...
public class TestPanel extends JPanel {
	...
	private GroupBookInterface groupBookIf;
	...
	private void initAddressBookIf() throws Exception {
		...
		if (o instanceof GroupBookInterface) {
			groupBookIf = (GroupBookInterface)o; 
		}
	}
	...
	private void initData() {
		...
		for (int i = 0; i < groupBookList.size(); i++) {
			...
			GroupBookVo oneGroup = groupBookList.get(i);
			...
			if(oneGroup.getParent_group_id() == 0) {				
				...
				for (int j = 0; j < parents.size(); j++) {
					if (((GroupBookVo) parents.get(j).getUserObject()).getGroup_id() == oneGroup.getParent_group_id()) {
	...
	public void jButton_Save_actionPerformed(ActionEvent e) {
		...
		try {			
			groupBookIf.insertGroupBook(oneGroup);
			treeReset();
			textFieldReset();
		}
		catch (Exception ex) {
	...
	public void jButton_Delete_actionPerformed(ActionEvent e) {
		...
		GroupBookVo p = new GroupBookVo();
		p.setGroup_id(Integer.parseInt(groupId));

		try {
			groupBookIf.deleteGroupBook(p);	
			treeReset();
			textFieldReset();
		}
		catch (Exception ex) {
	...
	public void jButton_Update_actionPerformed(ActionEvent e) {
		...
		try {			
			groupBookIf.updateGroupBook(oneGroup);
			treeReset();
			textFieldReset();
		}
		catch (Exception ex) {
